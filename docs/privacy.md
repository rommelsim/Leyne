...
information. This policy explains exactly what happens to data when you use
the app.
...
Leyne requests **"While Using the App"** location access for one purpose: to
find and sort bus stops near you. Your location:

- Is processed **on device only**;
...
Live arrivals, stops, services, and routes are retrieved from the
**Land Transport Authority (LTA) DataMall** API. These network requests
include only non-personal identifiers such as a bus stop code or service
number. No device or personal data is added by the app. LTA's handling of
that API traffic is governed by LTA's own terms and privacy practices.
...
- No user accounts, profiles, or sign-in.
- No analytics or tracking SDKs other than what AdMob requires for ads.
- No selling or renting of data.
- No collection of contacts, photos, health, or financial data.
...
If this policy changes, we will update the "Effective date" above and post
the revised policy at this URL before the change takes effect.

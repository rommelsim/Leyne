...
class RouteTimeline extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final t = context.t;
    final aheadIdx = stops.indexWhere((s) => s.state == SoftRouteStopState.here);
    final aheadCount = aheadIdx >= 0 ? stops.length - aheadIdx - 1 : 0;
    final showHint =
        alightId == null && stops.any((s) => s.state == SoftRouteStopState.next);
        ...
        children: [
          ...
          if (showHint)
            Padding(
              ...
              child: Text('Tap a stop to be alerted when arriving.',
  ...
  Widget _row(BuildContext context, SoftRouteStop stop, bool first, bool last) {
    final t = context.t;
    final upcoming = stop.state == SoftRouteStopState.next ||
              ...
              child: Stack(alignment: Alignment.center, children: [
                Column(children: [
                  Expanded(
                    child: Container(
                      width: 2,
                      color: first ? Colors.transparent : _connector(t, resolved),
                    ),
                  ),
                  Expanded(
                    child: Container(
                      width: 2,
                      color: last ? Colors.transparent : _connector(t, resolved),
                    ),
                  ),
                ]),
                _dot(t, resolved),
                ...
                child: Column(
                  ...
                  children: [
                    Row(children: [
                      ...
                      if (resolved == SoftRouteStopState.next && stop.etaMin != null)
                        Text(_clockETA(stop.etaMin!),
                    ...
                    if (resolved == SoftRouteStopState.board)
                      _chip(t, 'BOARD', filled: true),
                    if (resolved == SoftRouteStopState.alight)
                      _chip(t, '🔔 ALIGHT', filled: true),
                  ],
                ),
              ),
            ),
      ...
      case SoftRouteStopState.alight:
        return Stack(alignment: Alignment.center, children: [
          Container(
              width: 18, height: 18,
              decoration: BoxDecoration(
                  color: t.accent.withValues(alpha: 0.25), shape: BoxShape.circle)),
          Container(
              width: 10, height: 10,
